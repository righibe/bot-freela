...
class PerfilProfissionalModal(Modal, title='Passo 1 — Seus Dados'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Trava de idempotência: evita duas threads de verificação num duplo-submit.
        with guarda_acao(('verificacao', user.id)) as livre:
            if not livre:
                await interaction.response.send_message('⏳ Sua verificação já está sendo iniciada. Aguarde...', ephemeral=True)
                return
